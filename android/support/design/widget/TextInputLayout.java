...
public class TextInputLayout extends LinearLayout {
    ...
    class C02774 implements AnimatorUpdateListener {
        C02774() {
        }
    ...
    class C03382 extends ViewPropertyAnimatorListenerAdapter {
        C03382() {
        }
    ...
    class C03393 extends ViewPropertyAnimatorListenerAdapter {
        final /* synthetic */ CharSequence val$error;

        C03393(CharSequence charSequence) {
        ...
        if (!TextUtils.equals(this.mError, error)) {
            ...
            if (this.mErrorShown) {
                ...
                if (animate) {
                    ...
                    ViewCompat.animate(this.mErrorView).alpha(1.0f).setDuration(200).setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR).setListener(new C03382()).start();
                }
            } else if (this.mErrorView.getVisibility() == 0) {
                if (animate) {
                    ViewCompat.animate(this.mErrorView).alpha(0.0f).setDuration(200).setInterpolator(AnimationUtils.FAST_OUT_LINEAR_IN_INTERPOLATOR).setListener(new C03393(error)).start();
                } else {
                    this.mErrorView.setVisibility(4);
                }
            }
    ...
    private void animateToExpansionFraction(float target) {
        if (this.mCollapsingTextHelper.getExpansionFraction() != target) {
            if (this.mAnimator == null) {
                ...
                this.mAnimator.setUpdateListener(new C02774());
            }
